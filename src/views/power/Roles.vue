<template>
  <div>
   ...
   <!-- <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item v-for="(item,index) in barn" :key="index"
 ...
 -->
      ...
      <el-row>
        <el-col>
         <!-- <el-button type="primary" @click="AddRoleDialogVisible = true">添加角色</el-button> -->
          <el-button type="primary" @click="addGsForm">添加角色</el-button>
         </el-col>
      </el-row>
      <!-- 角色列表 -->
        ...
        <el-table-column label="操作" width="300px">
          <template slot-scope="scope">
            <!-- <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row.id)">编辑</el-button> -->
             <el-button type="primary" icon="el-icon-edit" size="mini" @click="bjGsForm(scope.row)">编辑</el-button>
    ...
    <!-- 添加角色对话框 -->
    <el-dialog :title="title"  width="40%" :visible.sync="dialogAddgsVisible"  @close="closeDialogAddgsVisible">
...
<script>
  import { rolseList,addInface,editFun} from '../../api/roles';
export default {
  data () {
    return {
     addRoleForm:{
  ...
  methods: {
    ...
    bjGsForm(val){
            ...
            this.dialogAddgsVisible = true;
            this.title="编辑"
            this.addRoleForm.roleName=val.roleName
            this.addRoleForm.roleDesc=val.roleDesc
            this.addRoleForm.id=val.id
       },

       //关闭或取消弹框
       closeDialogAddgsVisible(){
        this.$refs.addRoleFormRef.resetFields();//element封装的方法,清空模态框的值
        this.title="" //初始化title的值
        this.addRoleForm={//初始化addForm中的值
             roleName:"",
             roleDesc:"",
             id:""
         }
        ...
        saveAddForm() {
          let _self=this
           _self.$refs.addRoleFormRef.validate(async valid => {
           if (valid) {
               let params = {
                   id: _self.addRoleForm.id,
                   roleName: _self.addRoleForm.roleName,
                   roleDesc: _self.addRoleForm.roleDesc,
               };
               if(!params.id){//当没有传过来id的时候,说明是添加,所以发送添加请求
                     const res =await addInface({roleName:params.roleName,roleDesc:params.roleDesc})
                     console.log('add')
                     _self.$message.success('添加角色成功！')
                     _self.dialogAddgsVisible = false
                     _self.getRolesList()
               }else{//发送编辑请求
                      const res =await editFun(params,params.id)
                      console.log('edit')
                      _self.$message.success('修改角色成功！')
                      _self.dialogAddgsVisible = false
                      _self.getRolesList()
               }
             }
    ...
    getRouterBarn () {
      ...
      let arr = []
      b.forEach((v, k) => {
        if (v.name === 'home' || v.name === 'roles') return
        arr.push({
          name: v.name,
          // path: v.path,
          title: v.meta.title
    ...
    async getRolesList () {
      const { data: res } = await rolseList()
